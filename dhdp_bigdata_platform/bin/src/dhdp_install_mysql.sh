...
#mysql的默认安装目录
mysql_install_dir=/home/hadoop/core
#mysql数据文件存放目录
mysql_data_dir=/home/hadoop/data/mysql
#mysql日志存放目录
mysql_log_dir=/home/hadoop/logs/mysql
#mysql安装包存放位置
mysql_install_package=/home/hadoop/package
#MySQL配置文件存放目录
mysql_conf=/home/hadoop/conf/mysql
...
function install_each_host_mysql(){
        ...
        #4.上传mysql-$1-linux-glibc2.12-x86_64.tar.gz文件到$install_package下，并进行解压
        if [ ! -f $mysql_install_package/$mysql_install_package_name.tar.gz ];
        then 
        ...
        #6.创建安装mysql使用的用户组和用户hadoop
          #groupadd hadoop && useradd -r -g hadoop hadoop

        #7.更改指定mysql安装包文件夹及数据库数据文件目录的用户组信息
	 ...
	 #15.删除/etc/my.cnf文件中的免密权限校验属性skip-grant-tables并重启服务
        ...
        #16.重启使skip-grant-tables权限校验生效并测试mysql是否安装成功
        /etc/init.d/mysqld restart
        if [ $? -eq 0 ];
                then 
                #验证hadoop用户登录并使用mysql是否成功
                mysql -uhadoop -phadoop -e "show databases"
                if [ $? -eq 0 ];
                then
        ...
        function main(){
                if [ -z "$1" ] || [ -z "$2" ];
                        then 
                                echo '[-] Usage: ciyu_install_each_host_component.sh [mode] [mysql_version]'
